...
class TestPriceCache:
    ...
    def test_timestamp_none_is_not_falsy_zero(self):
        """Test that timestamp=0.0 is not replaced by time.time()."""
        cache = PriceCache()
    ...
    def test_session_open_set_on_first_update(self):
        """Test session_open is recorded on the first price update."""
        cache = PriceCache()
        cache.update("AAPL", 190.00)
        assert cache.get_session_open("AAPL") == 190.00

    def test_session_open_not_overwritten(self):
        """Test session_open stays at the first-seen price on subsequent updates."""
        cache = PriceCache()
        cache.update("AAPL", 190.00)
    ...
    def test_session_open_cleared_on_remove(self):
        """Test session_open is cleared when the ticker is removed."""
        cache = PriceCache()
        cache.update("AAPL", 190.00)
        cache.remove("AAPL")
        assert cache.get_session_open("AAPL") is None

    def test_session_open_unknown_ticker(self):
        """Test session_open returns None for a ticker not yet seen."""
        cache = PriceCache()
        assert cache.get_session_open("AAPL") is None
